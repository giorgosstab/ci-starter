...
class Ion_auth_model extends CI_Model
{
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$this->increase_login_attempts($identity,!empty($user->id) ? $user->id : 0);
	 ...
	 * @return bool
	 */
	public function increase_login_attempts($identity,$user_id)
	{
		if ($this->config->item('track_login_attempts', 'ion_auth'))
		{
			...
			if ($this->config->item('logger_feature', 'ion_auth')) {
				if($user_id) {
					$this->logger
					->user($user_id)
					...
					->token('login_attempt')
					->id($user_id)
					->comment('Failure login user!')
					->log();
				} else {
					$message = 'Failure login '.$identity.'!';
					$this->logger
						->user('0')
						->ip($this->input->ip_address())
						->type('Auth')
